...
bool ResampleCore(const System::SharedPtr<ImageData>& imageData, System::Drawing::SizeF shapeSizeInPoints, int32_t ppi, int32_t jpegQuality)
{
    ...
    try
    {
        ...
        System::SharedPtr<System::Drawing::Image> srcImage = imageData->ToImage();
        {
            ...
            System::SharedPtr<System::Drawing::Bitmap> dstImage = System::MakeObject<System::Drawing::Bitmap>(dstWidthPixels, dstHeightPixels);
            {
                ...
                std::cout << "Original size " << originalBytes->get_Length() << ", new size " << dstStream->get_Length() << ".\n";
    ...
    catch (System::Exception& e)
    {
        ...
        std::cout << "Error processing an image, ignoring. " << e.get_Message().ToUtf8String() << '\n';
    }
...
void CompressImages()
{
    ...
    std::cout << "Loading " << srcFileName.ToUtf8String() << ". Size " << GetFileSize(srcFileName) << ".\n";
    ...
    std::cout << "Resampled " << count << " images.\n";
    
    if (count != 1)
    {
        std::cout << "We expected to have only 1 image resampled in this test document!\n";
    }
    
    System::String dstFileName = dataDir + GetOutputFilePath(u"CompressImages.doc");
    doc->Save(dstFileName);
    std::cout << "Saving " << dstFileName.ToUtf8String() << ". Size " << GetFileSize(dstFileName) << ".\n";
    ...
    std::cout << "\nCompressed images successfully.\nFile saved at " << dstFileName.ToUtf8String() << '\n';
}
