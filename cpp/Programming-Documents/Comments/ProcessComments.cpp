...
void CommentResolvedandReplies(const System::SharedPtr<Document>& doc)
{
    ...
    while (childComment_enumerator->MoveNext() && (childComment = System::DynamicCast<Comment>(childComment_enumerator->get_Current()), true))
    {
        ...
        std::cout << childComment->get_Ancestor()->get_Id() << '\n'
            << childComment->get_Done() << '\n';
...
void ProcessComments()
{
    ...
    std::cout << "Comments from \"pm\" are removed!\n";
    ...
    // Remove all comments.
    RemoveComments(doc);
    std::cout << "All comments are removed!\n";
    
    dataDir = dataDir + GetOutputFilePath(u"ProcessComments.doc");
    // Save the document.
    doc->Save(dataDir);
