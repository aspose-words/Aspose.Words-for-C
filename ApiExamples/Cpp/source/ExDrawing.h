...
public:
    ...
    void TypeOfImage()
    {
...

        {
...

            {
                ...
                ASSERT_EQ(ImageType::Png, imgShape->get_ImageData()->get_ImageType());
            }
        }
        //ExEnd
    }
    ...
    void SaveAllImages()
    {
        ...
        auto shapes = imgSourceDoc->GetChildNodes(NodeType::Shape, true)->LINQ_Cast<SharedPtr<Shape>>()->LINQ_Where([](SharedPtr<Shape> s) { return s->get_HasImage(); });
        ...
        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<String(String s)> _local_func_2 = [](String s)
        {
            return s;
        };

        ArrayPtr<String> imageFileNames = System::IO::Directory::GetFiles(ArtifactsDir)
                                                              ->LINQ_Where(static_cast<System::Func<String, bool>>(_local_func_1))
                                                              ->LINQ_OrderBy(static_cast<System::Func<String, String>>(_local_func_2))
                                                              ->LINQ_ToArray();

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<SharedPtr<System::IO::FileInfo>(String s)> _local_func_3 = [](String s)
        {
            return MakeObject<System::IO::FileInfo>(s);
        };

        SharedPtr<System::Collections::Generic::List<SharedPtr<System::IO::FileInfo>>> fileInfos =
            imageFileNames->LINQ_Select(static_cast<System::Func<String, SharedPtr<System::IO::FileInfo>>>(_local_func_3))->LINQ_ToList();
    ...
    void ImportImage()
    {
        ...
        auto imgShape = MakeObject<Shape>(doc, ShapeType::Image);
    ...
    /// </summary>
    class ShapeInfoPrinter : public DocumentVisitor
    {
    public:
        ShapeInfoPrinter()
        {
...
protected:
    void TestGroupShapes(SharedPtr<Document> doc)
    {
