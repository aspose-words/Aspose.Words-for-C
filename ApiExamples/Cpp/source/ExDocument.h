...
public:
    ...
    void AppendDocumentFromAutomation()
    {
        ...
        for (int i = 1; i <= recordCount; i++)
        {
            ...
            std::function<bool()> _local_func_1 = [&srcDoc]()
            {
                return srcDoc == MakeObject<Document>(u"C:\\DetailsList.doc");
            };

            ASSERT_THROW(_local_func_1(), System::IO::FileNotFoundException);
            ...
            if (i > 1)
            {
                ...
                // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
                std::function<void()> _local_func_2 = [&doc, &i]()
                {
                    doc->get_Sections()->idx_get(i)->get_HeadersFooters()->LinkToPrevious(false);
                };

                ASSERT_THROW(_local_func_2(), System::NullReferenceException);
            }
        }
    }
    ...
    void DigitalSignature_()
    {
        ...
        DigitalSignatureUtil::Sign(MyDir + u"Document.docx", ArtifactsDir + u"Document.DigitalSignature.docx", certificateHolder,
                                   [&]
                                   {
    ...
    void AppendAllDocumentsInFolder()
    {
        ...
        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(String item)> _local_func_3 = [](String item)
        {
            return item.EndsWith(u".doc");
        };

        SharedPtr<System::Collections::Generic::List<String>> docFiles =
            System::IO::Directory::GetFiles(MyDir, u"*.doc")->LINQ_Where(static_cast<System::Func<String, bool>>(_local_func_3))->LINQ_ToList();
        ...
        ASSERT_EQ(8, dstDoc->get_Sections()->get_Count());
    }
    ...
    void CompareDocumentWithRevisions()
    {
        ...
        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<void()> _local_func_4 = [&docWithRevision, &doc1]()
        {
            docWithRevision->Compare(doc1, u"John Doe", System::DateTime::get_Now());
        };

        ASSERT_THROW(_local_func_4(), System::InvalidOperationException);
    }
    ...
    void CompareOptions_()
    {
        ...
        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_5 = [](SharedPtr<Revision> rev)
        {
            return rev->get_RevisionType() == RevisionType::FormatChange;
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreFormatting(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_5)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> s)> _local_func_6 = [](SharedPtr<Revision> s)
        {
            return s->get_ParentNode()->GetText().Contains(u"hello");
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreCaseChanges(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_6)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_7 = [](SharedPtr<Revision> rev)
        {
            return HasParentOfType(rev, NodeType::Comment);
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreComments(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_7)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_8 = [](SharedPtr<Revision> rev)
        {
            return HasParentOfType(rev, NodeType::Table);
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreTables(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_8)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_9 = [](SharedPtr<Revision> rev)
        {
            return HasParentOfType(rev, NodeType::FieldStart);
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreFields(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_9)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_10 = [](SharedPtr<Revision> rev)
        {
            return HasParentOfType(rev, NodeType::Footnote);
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreFootnotes(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_10)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_11 = [](SharedPtr<Revision> rev)
        {
            return HasParentOfType(rev, NodeType::Shape);
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreTextboxes(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_11)));

        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<bool(SharedPtr<Revision> rev)> _local_func_12 = [](SharedPtr<Revision> rev)
        {
            return HasParentOfType(rev, NodeType::HeaderFooter);
        };

        ASPOSE_ASSERT_NE(compareOptions->get_IgnoreHeadersAndFooters(),
                         docOriginal->get_Revisions()->LINQ_Any(static_cast<System::Func<SharedPtr<Revision>, bool>>(_local_func_12)));
    }
...
public:
    ...
    void HyphenationOptionsExceptions()
    {
        ...
        // CSPORTCPP: [WARNING] Using local variables. Make sure that local function ptr does not leave the current scope.
        std::function<void()> _local_func_13 = [&doc]()
        {
            doc->get_HyphenationOptions()->set_HyphenationZone(0);
        };

        ASSERT_THROW(_local_func_13(), System::ArgumentOutOfRangeException);
    ...
    void UseSubstitutions()
    {
        ...
        ASSERT_EQ(doc->GetText(), u"Paul take money from Jason.\f");
        //ExEnd
    }
    ...
    void DocPackageCustomParts()
    {
        ...
        {
            SharedPtr<System::Collections::Generic::IEnumerator<SharedPtr<CustomPart>>> enumerator =
                doc->get_PackageCustomParts()->GetEnumerator();
            int index = 0;
            while (enumerator->MoveNext())
            {
                ...
                std::cout << (enumerator->get_Current()->get_IsExternal() ? u"\tSourced from outside the document"
                                                                          : String::Format(u"\tStored within the document, length: {0} bytes",
                                                                                                   enumerator->get_Current()->get_Data()->get_Length()))
                          << std::endl;
                index++;
            }
        }
...
public:
    ...
    void ReadMacrosFromExistingDocument()
    {
        ...
        //ExSkip
        std::cout << (vbaProject->get_IsSigned()
                          ? String::Format(u"Project name: {0} signed; Project code page: {1}; Modules count: {2}\n", vbaProject->get_Name(),
                                                   vbaProject->get_CodePage(), vbaProject->get_Modules()->LINQ_Count())
    ...
    void GetWebExtensionInfo()
    {
        ...
        {
            SharedPtr<System::Collections::Generic::IEnumerator<SharedPtr<WebExtensionProperty>>> enumerator =
                webExtensionPropertyCollection->GetEnumerator();
            while (enumerator->MoveNext())
            {
