...
public:
    ...
    void ImportNode()
    {
        ...
        dstDoc->get_FirstSection()->get_Body()->get_FirstParagraph()->AppendChild(
            MakeObject<Run>(dstDoc, u"Destination document first paragraph text."));
    ...
    void BackgroundShape()
    {
        ...
        {
            doc->set_BackgroundShape(MakeObject<Shape>(doc, ShapeType::Triangle));
        };
...
private:
    ...
    public:
        ResourceLoadingAction ResourceLoading(SharedPtr<ResourceLoadingArgs> args) override
        {
            ...
            {
                const String& switch_value_0 = args->get_OriginalUri();
                if (switch_value_0 == u"Google logo")
                {
                    {
                ...
                else if (switch_value_0 == u"Aspose logo")
                {
                    {
                        auto webClient = MakeObject<System::Net::WebClient>();
...
protected:
    void TestResourceLoadingCallback(SharedPtr<Document> doc)
    {
        ...
        TestUtil::VerifyWebResponseStatusCode(System::Net::HttpStatusCode::OK, AsposeLogoUrl);
    }
};
